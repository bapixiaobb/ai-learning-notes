...
- 信息只向前流动；
- 参数通过 training 被学习；
- 通常用 [[Backpropagation]] 计算 gradients。

>[!note]
>你之前那张 [[Neural Network]] 卡讲的是神经网络这个大类。
>
...
| [[Transformer]] | 通过 attention 在 sequence positions 间交互 | Self-Attention |
...
在 [[Transformer Block]] 中，Feed-Forward Network 通常也叫 [[MLP]]。
...
>
>Token positions 之间的信息交换主要由 [[Self-Attention]] 完成。
...
在 [[Original Transformer]] 中，FFN 常用 [[ReLU]]

>[!note]  
所以在 LLM 语境里，[[Feed-Forward Network]]、[[MLP]]、FFN 经常指 Transformer block 中 attention 后面的 per-token nonlinear module。

## **🚫 Common Confusions**

### **1. Feed-forward 不等于 forward pass**

[[Forward Pass in Transformer]] 指一次模型前向计算过程。  
[[Feed-Forward Network]] 指一种 neural network structure。
...
真正让 token positions 之间交换信息的是 [[Self-Attention]]。
