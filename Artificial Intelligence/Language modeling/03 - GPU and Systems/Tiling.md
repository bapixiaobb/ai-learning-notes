...
是 [[GPU Memory Bound]] 的一种重要的针对 Matmul 的优化方法

>[!important] Intuition
>Tiling = 把大矩阵乘法拆成小块，让每一小块数据搬进快 memory 后，被尽可能多次复用，再写回慢 memory。

它不是单纯“切块”，而是适配 [[GPU]] 类似硬件的方法：
```
切块
↓
...
比如你想读一个二维 tile：一行一行连续读，如果每一行刚好对齐 memory burst window，读取很舒服。但如果矩阵宽度偏了 1，下一行起始地址就错位了。![[Memory Alignment.png]]
