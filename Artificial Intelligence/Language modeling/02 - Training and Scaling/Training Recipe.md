...
>
>[Decoder-Only Transformer](<../../Transformer/Decoder-Only%20Transformer.md>) 用 Causal Mask 保证每个位置只能看见 prefix，然后通过 Language Modeling Head 输出 next-token logits。
...
[Gradient Accumulation](<./Gradient%20Accumulation.md>) 用来在显存不足以容纳大 batch 的情况下，模拟更大的 effective batch size。
