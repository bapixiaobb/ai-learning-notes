...
![[gpu-node-overview.png]]

多 GPU 训练可以理解成把 [[GPU]] 的 memory hierarchy 往外扩展：
...
所以 [[Parallelism]] 的问题不是只看“能不能切”，而是要看切完之后 ranks 之间需要通信多少、通信路径有多快。

[[NVIDIA Collective Communication Library|NCCL]] 负责把 collective operations（例如 all-reduce、all-gather、reduce-scatter）映射到底层 GPU 通信拓扑上，让多张 GPU 能高效交换 tensor。
