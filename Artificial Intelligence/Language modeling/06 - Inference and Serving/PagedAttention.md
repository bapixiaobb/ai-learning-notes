...
![paged-attention-fragmentation.png](<../../attachments/paged-attention-fragmentation.png>)
...
![paged-attention-blocks.png](<../../attachments/paged-attention-blocks.png>)
维护一张 block table，上面的 Block 0 就是 Logical Block（顺序的），所以👆虽然 Physical Block 乱序，request 仍然认为自己的 sequence 是：
```
Four → score → and → seven → years → ago → our → fathers → brought → forth
```

所以这种情况下，在 [Decode](<Decode.md>) 的过程中，不需要在一个满的 block 后面找空间，直接找一个 空闲的 Physical Block 就好
...
![paged-attention-logical.png](<../../attachments/paged-attention-logical.png>)
...
这个想法比较直观
![paged-attention-parallel.png](<../../attachments/paged-attention-parallel.png>)
