...
>
>它通过 [Self-Attention](<./Self-Attention.md>) 建模序列内部 token 之间的关系，并通过 encoder-decoder structure 实现从 source sequence 到 target sequence 的生成。
...
3. [Residual Connection](<./Residual%20Connection.md>)
4. [Layer Normalization](<./Layer%20Normalization.md>)
...
这和 [Decoder-Only Transformer](<./Decoder-Only%20Transformer.md>) 中的 [Causal Attention](<./Causal%20Attention.md>) 不同。
...
4. [Residual Connection](<./Residual%20Connection.md>)
5. [Layer Normalization](<./Layer%20Normalization.md>)
...
这需要 [Causal Mask](<./Causal%20Mask.md>)。
...
因此需要加入 [Positional Encoding](<./Positional%20Encoding.md>)。
...
现代 LLM 中更常见的是 [Rotary Position Embedding](<./Rotary%20Position%20Embedding.md>)，但这不是 Original Transformer 的原始设计。
...
这对应 [Post-Norm Transformer](<./Post-Norm%20Transformer.md>)。

>**Note**
>Original Transformer 使用的是 Post-Norm 结构。
>
>现代 LLM 中更常见的是 [Pre-Norm Transformer](<./Pre-Norm%20Transformer.md>)，例如先 norm 再进入 attention / MLP。

Residual connection 的作用是让信息和 gradient 更容易穿过深层网络，但 [Residual Stream](<./Residual%20Stream.md>) 这个概念在 modern Transformer 解释中会更突出。
...
现代 LLM 中常见的 [SwiGLU](<./SwiGLU.md>) 不是 Original Transformer 的原始配置，而是后来的 architecture choice。
