...
[[Residual Connection]] 里的 “residual” 和 regression 里的 “residual” 不是同一个概念。  
...
|---|---|---|
| [[Residual Connection]] | $x + F(x)$ | 网络层内部的信息流结构 |
...
在 [[Transformer Block]] 中，residual connection 的作用非常核心。
...
>[!note]
>Transformer 中的 [[Residual Stream]] 就是这些 residual connections 串联后形成的主 hidden state 流。
>
...
>
>[[Residual Stream]] 不是预测误差，也不是 loss 的 residual。
>
...
>[[Residual Connection]] 和 regression residual 都用了 “residual” 这个词，但它们不是同一个概念。
>
>在 Transformer 中，residual connection 表示保留当前 hidden state，并加上 attention 或 MLP 计算出的更新量。
>
>在 regression 中，residual 表示真实值和预测值之间的差。
>
>所以 Transformer 的 [[Residual Stream]] 不是误差流，而是 hidden representation 的主数据流。
