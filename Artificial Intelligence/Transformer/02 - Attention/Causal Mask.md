...
Causal Mask 是实现 [Self-Causal Attention](<Self-Causal%20Attention.md>) 的 attention mask。
...
这和 [Autoregressive Decoding](<../../Language%20modeling/02%20-%20Language%20Modeling%20Basics/Autoregressive%20Decoding.md>) 的建模目标一致：
...
在训练 [Decoder-Only Transformer](<../00%20-%20Maps%20and%20Architectures/Decoder-Only%20Transformer.md>) 时，整个 sequence 通常会一次性输入模型：

```math
[x_1, x_2, x_3, x_4]
```

如果没有 mask，第 1 个位置可能直接看到 $x_2, x_3, x_4$。 如果模型要预测：
```math
x_4
```
它只能使用：
...
x_4, x_5, \dots
```
训练就会变成作弊，这个机制契合了 [Next-token prediction](<../../Language%20modeling/02%20-%20Language%20Modeling%20Basics/Next-token%20prediction.md>)。
...
## 🎭 In Self-Attention: [Self-Causal Attention](<Self-Causal%20Attention.md>)
...
[Self-Causal Attention](<Self-Causal%20Attention.md>) 指带有 causal constraint 的 self-attention。
