...
| [activation](<../../Neural%20Networks/Activations.md>) | 对 neural network hidden states normalization |
...
[Softmax](<../02%20-%20Attention/Softmax.md>) 也可以看作一种把 arbitrary logits 转成 probability distribution 的 normalization：
...
>
>例如 [Layer Normalization](<Layer%20Normalization.md>) 会 subtract mean，但 [RMSNorm](<RMSNorm.md>) 不 subtract mean，只做 scaling。
...
更常见的还有 z-score、min-max、L2 normalization、[Softmax](<../02%20-%20Attention/Softmax.md>) normalization 等。
...
>
>不同场景下 normalization 的公式不同：z-score normalization 控制 mean 和 variance，min-max normalization 控制取值区间，L2 normalization 控制向量长度，[Softmax](<../02%20-%20Attention/Softmax.md>) normalization 把 logits 转成概率分布。
>
