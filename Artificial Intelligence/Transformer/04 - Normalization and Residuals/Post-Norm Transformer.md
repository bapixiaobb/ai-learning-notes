...
一个 Post-Norm [Transformer Block](<../00%20-%20Maps%20and%20Architectures/Transformer%20Block.md>) 通常有两次 Post-Norm update。
...
[Original Transformer](<../00%20-%20Maps%20and%20Architectures/Original%20Transformer.md>) 使用的是 Post-Norm 结构。
...
[Original Transformer](<../00%20-%20Maps%20and%20Architectures/Original%20Transformer.md>) 是 encoder-decoder structure，并且使用 Post-Norm。
但 Post-Norm 本身只是 block 内部的 norm placement。

### 3. Post-Norm 不改变 attention mask

Post-Norm 只决定 norm 放在哪里。
是否 causal 由 [Causal Mask](<../02%20-%20Attention/Causal%20Mask.md>) 或 attention pattern 决定。
...
>
>[Original Transformer](<../00%20-%20Maps%20and%20Architectures/Original%20Transformer.md>) 使用的是 Post-Norm，而 modern decoder-only LLM 更常用 [Pre-Norm Transformer](<Pre-Norm%20Transformer.md>)。
>
>理解 Post-Norm 的关键是：
>
