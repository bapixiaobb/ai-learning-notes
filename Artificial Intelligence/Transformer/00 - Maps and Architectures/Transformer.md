...
它的核心思想是：用 [Self-Attention](<../02%20-%20Attention/Self-Attention.md>) 让序列中每个 token 根据上下文更新自己的 representation。
...
![transformer model.png](<../../attachments/transformer%20model.png>)
...
[Self-Attention](<../02%20-%20Attention/Self-Attention.md>) 是 Transformer 的核心机制。
...
Self-attention 的具体公式和 $Q,K,V$ 机制放在 [Self-Attention](<../02%20-%20Attention/Self-Attention.md>) 中单独整理。
...
>
>它的核心是用 [Self-Attention](<../02%20-%20Attention/Self-Attention.md>) 在 token positions 之间进行信息交互，再通过多层 [Transformer Block](<Transformer%20Block.md>) 不断更新 token representations。
>
...
这张图沿着 tensor shapes 展示了 Transformer LM 的 forward，终点是 `[batch, seq, vocab_size]` 的 logits。Training 时，logits 会和 shifted next-token targets 一起交给 [Loss Function](<../../Neural%20Networks/Loss%20Function.md>)；loss function 才负责判断这次 prediction 有多差，并把 model forward 接到 backpropagation 和 optimizer。
