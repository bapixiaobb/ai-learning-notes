...
Learning Rate 就是 [Optimizer](<Optimizer.md>) 中 gradient descent 的 step size

在训练 [Transformer](<../00%20-%20Maps%20and%20Architectures/Transformer.md>) 模型时，能使 Loss 下降最快的 step size 不是固定的，不同 training stages 适合的 update scale 不同，因此 learning rate 通常随 optimizer step 变化。所以我们要做 Learning Rate Schedule。一般是：
```math
