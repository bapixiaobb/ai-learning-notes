...
每个 head 都有自己的 Q、K、V projections，可以从不同 representation subspace 学习不同的 token interaction pattern。实现上通常用一个大的 projection matrix 一次性算出所有 heads 的 Q/K/V，然后 reshape/split 出 head dimension。详见 [Implementation of Multi-head Attention](<./Implementation%20of%20Multi-head%20Attention.md>)
...
Multi-Head Attention 通常是在 [Self-Attention](<./Self-Attention.md>) 上做多头扩展。
...
如果再加上 [Causal Mask](<./Causal%20Mask.md>)，就得到 causal multi-head attention：
...
在 [Decoder-Only Transformer](<./Decoder-Only%20Transformer.md>) 中，Multi-Head Attention 通常是 causal 的。

## 🦙 Relation to Modern LLMs

[Original Transformer](<./Original%20Transformer.md>) 使用 standard Multi-Head Attention。
...
>
>所以 attention head 的设计既是 [Model Architecture](<../Language%20modeling/05%20-%20Architectures%20and%20MoE/Model%20Architecture.md>) 问题，也会影响 Systems for Language Models。
