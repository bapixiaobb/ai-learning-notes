...
[Softmax](<./Softmax.md>) 也可以看作一种把 arbitrary logits 转成 probability distribution 的 normalization：
...
更常见的还有 z-score、min-max、L2 normalization、[Softmax](<./Softmax.md>) normalization 等。
...
>
>不同场景下 normalization 的公式不同：z-score normalization 控制 mean 和 variance，min-max normalization 控制取值区间，L2 normalization 控制向量长度，[Softmax](<./Softmax.md>) normalization 把 logits 转成概率分布。
>
