...
[[Pre-Norm Transformer]] 指把 normalization 放在 sublayer 之前的 Transformer block 结构。
...
- $\mathrm{Norm}$ 可以是 [[Layer Normalization]] 或 [[RMSNorm]]；
- $F$ 是某个 sublayer，例如 [[Self-Attention]] 或 [[MLP]]；
- $x + F(\mathrm{Norm}(x))$ 是 [[Residual Connection]]。
...
一个 Pre-Norm [[Transformer Block]] 通常有两次 Pre-Norm update：
...
在 Pre-Norm Transformer 中，[[Residual Stream]] 有一条比较直接的路径穿过所有 layers。
...
>
>Modern decoder-only LLM 更常使用 Pre-Norm，常见搭配是 [[Pre-Norm Transformer]] + [[RMSNorm]]。

## 🦙 In Llama-style Architecture

在 [[Llama-style Architecture]] 中，Pre-Norm 通常和 [[RMSNorm]] 搭配。
...
- [[RMSNorm]] 是 normalization method；
- Pre-Norm 是 normalization placement；
- [[Residual Connection]] 是加法结构；
- [[Residual Stream]] 是主 hidden state 流。
...
[[RMSNorm]] 是 normalization formula。  
[[Pre-Norm Transformer]] 是 block layout。
...
是否 causal 由 [[Causal Attention]] 和 [[Causal Mask]] 决定。
