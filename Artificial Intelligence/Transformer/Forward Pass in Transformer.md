...
>
>参数更新发生在 backward pass 和 [[Optimizer]] step 中。Forward pass 只是在当前参数下计算 activations、hidden states、logits 和 loss。
...
[[Token Embedding]] 会把每个 token id 映射成 dense vector：
...
在 [[Original Transformer]] 中，position information 通常通过 positional encoding 加到 token embeddings 上：
...
在 modern LLM 中，常见的是 [[Rotary Position Embedding]]。  
...
embedding 之后得到的 hidden states 可以看作初始 [[Residual Stream]]：
...
这个 residual stream 会被送入第一层 [[Transformer Block]]。
...
Transformer 的主体是多个 [[Transformer Block]]。
...
经过 mask 和 [[Softmax]] 后，对 $V$ 做加权求和，得到 attention output。

在 [[Decoder-Only Transformer]] 中，attention 通常是 [[Causal Attention]]，所以第 $t$ 个位置只能看见：
...
Attention output 会通过 [[Residual Connection]] 加回 residual stream，而不是直接成为整个模型的最终输出。
...
现代 LLM 中可能使用 [[SwiGLU]] 这样的 gated MLP。
...
>
>需要经过 [[Softmax]] 才能得到 next-token probability distribution。
...
这部分属于 [[Training Recipe]]，而不是 [[Model Architecture]] 本身。
...
Attention output 会加回 [[Residual Stream]]，再经过后续 MLP、后续 layers、final norm 和 output head，最终才得到 logits。
