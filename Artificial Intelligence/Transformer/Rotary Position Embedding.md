...
Rotary Position Embedding, 简称 RoPE, 是一种把 position information 注入 [[Self-Attention]] 的方法。它常见于 modern decoder-only LLM, 例如 [[Llama-style Architecture]]。
...
>[!note]
> 一个 token 在模型里先是一个 integer token id。经过 [[Token Embedding]] 后, 每个 token 会变成一个 vector。
...
>
> 代码里会直接按 pair 做旋转。具体实现见 [[RoPE Implementation]]。
...
| [[Causal Mask]] | 防止看到 future tokens |
| [[Causal Attention]] | 只能 attend to prefix 的 attention pattern |
